...
import json,logging,time,re,copy
...
class ElasticSearch:
    ...
    def __init__(self,*args,**kwargs):
        ...
        self.info = {}
        self.__conn()
        self.idxnm = config.ES.INDEX_NAME
        if not self.es.ping():
        ...
        for _ in range(3):
            try:
                if not self.__version():
                    ...
                    _ = self.es.update(index=self.idxnm, id=id, body=json.dumps(script, ensure_ascii=False),
    ...
    def init_index(self,fid):
        idxnm = f"rag-{fid}"
        if ELASTICSEARCH.indexExist(idxnm):
            return idxnm
        _ = ELASTICSEARCH.createIdx(idxnm, json.load(
            open(os.path.join(abs_dir_path, "mapping.json"), "r")))
        return idxnm

    def preprocess(self,infos:List):
        pass
