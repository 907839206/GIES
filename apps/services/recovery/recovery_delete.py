...
class LayoutRecovery:
    ...
    def draw_pic(self,ocr_res,shape):
        white_color = (255, 255, 255)
        image = np.full((shape[0], shape[1], 3), white_color, np.uint8)
        ...
        font = ImageFont.truetype(os.path.join(abs_path_dir,"SimHei.ttf"), font_size)
        for _,info in enumerate(ocr_res):
            bbox = info[0]
            position = [int(e) for e in bbox[-1]]
            text,score = info[1]
...
if __name__ == "__main__":
    ...
    model = LayoutRecovery(_model_path)
    image_list = [
        Image.open(filepath)
    ]
    ...
    _ocr = OCR(_model_path)
    print(f"type :{type(mask_image_list[0])}")
    _cv2_img = np.array(mask_image_list[0].convert('RGB'))
    ocr_res = _ocr(_cv2_img)
    print(f"ocr_res:{ocr_res}")
    _draw_img = model.draw_pic(ocr_res,_cv2_img.shape[:2])
    _draw_img.save("draw_text.jpg")
